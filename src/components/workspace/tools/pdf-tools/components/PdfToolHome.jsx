  ...
  return (
    <section className={styles.uploadCard}>
      ...
      <div className={styles.modeGrid}>
        {TOOL_CARDS.map((tool) => {
          const primaryIconSize = tool.secondaryIcon ? 24 : 34
          const secondaryIconSize = tool.secondaryIcon ? 24 : 22
          ...
          return (
            <button className={styles.modeButton} key={tool.id} onClick={() => onOpenTool(tool.id)} type="button">
              <div className={styles.pdfToolCardTop}>
                ...
                >
                  <WorkspaceIcon name={tool.icon} size={primaryIconSize} />
                  {tool.secondaryIcon ? (
                    <WorkspaceIcon className={styles.pdfToolCardIconSecondary} name={tool.secondaryIcon} size={secondaryIconSize} />
                  ) : null}
                </span>
