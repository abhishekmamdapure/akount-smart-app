...
export default function SplitMergeTool({ onUsage }) {
  ...
  async function handleProcess() {
    ...
    try {
      ...
      const message = error?.message || 'Unable to process split / merge.'
      setStatusTone('warning')
    ...
    <section className={styles.uploadCard}>
      ...
      {mode === 'split' ? (
        <>
            ...
            {splitMode === 'after_pages' ? (
              <div className={styles.pdfField}>
                ...
                <div className={styles.pdfInfoCard}>
                  {splitCuts.length > 0
                    ? splitCuts.map((cutIndex) => cutIndex + 1).join(', ')
                    : 'No split points selected yet'}
                </div>
              </div>
            ) : (
              <label className={styles.pdfField}>
          ...
          {isPreviewLoading ? <p className={styles.uploadHint}>Rendering thumbnails...</p> : null}
                  ...
                  <div className={styles.pdfPreviewItem} key={`split-thumb-${thumb.pageNumber}`}>
                    ...
                    {splitMode === 'after_pages' && index < splitPreview.thumbnails.length - 1 ? (
                      <button
                        ...
                        type="button"
                      >
                        <span>{splitCuts.includes(index) ? 'Split here' : 'Add split'}</span>
                      </button>
                    ) : null}
                  </div>
                ))}
